...
Step-by-step description of the scenario:
 - teardown: clean the interfaces
 - setup: set the interfaces to emulate uplink and downlink paths
 - captures: capture network traffic to analyze
 - download: start the download of resources
 - analyzes: analyze captured traffic
 - teardown: clean the interface
"""
...
def build(scenario, args, capture=False):
    ...
    teardowns = [
       teardown(
          scenario, 
          name='Clean {} on {} in {}'.format(interfaces, entity, mode), 
          entity=entity, 
          interfaces=interfaces, 
          mode=mode
       )
       for entity, mode in ((args.server, 'ingress'), 
                            (args.server, 'egress'),
                            (args.client, 'ingress'),
                            (args.client, 'egress'))
    ]
    ...
    setups = [
       ...
       for name, entity, iface, mode, bandwidth, delay in (('Set downlink path', args.server, args.down_iface, 'egress', args.bandwidth_server_to_client, args.delay_server_to_client),
                                                   ('Set uplink path', args.client, args.up_iface, 'egress', args.bandwidth_client_to_server, args.delay_client_to_server))
    ]
